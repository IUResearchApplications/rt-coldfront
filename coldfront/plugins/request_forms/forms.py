...
class SoftwareRequestForm(forms.Form):
    ...
    destination = forms.ChoiceField(
        choices=DESTINATION_CHOICES, label="Destination:", widget=forms.RadioSelect
    )
    ...
    vendor_url = forms.URLField(
        max_length=100, label="Vendor URL:", help_text="Vendor URL for source code/binaries"
    )
...
class StatsRequestForm(forms.Form):
    ...
    question_addressed = forms.CharField(
        label="What question are you trying to address?", widget=forms.Textarea
    )
