...
def get_publications(usernames):
    ...
    headers = {"apikey": ACADEMIC_ANALYTICS_API_KEY}

    publications = []
    for user_id in user_ids:
        client_faculty_id = eval(ACADEMIC_ANALYTICS_FORMULA.format(int(user_id)), {"__builtins__": {}}, {})
        person_results = requests.get(
            url=ACADEMIC_ANALYTICS_API_BASE_ADDRESS
            + f"GetPersonIdByClientFacultyId?clientFacultyId={client_faculty_id}",
            headers=headers,
        )
        ...
        articles_result = requests.get(
            url=ACADEMIC_ANALYTICS_API_BASE_ADDRESS + f"{person_id}/articles", headers=headers
        )
        ...
        proceedings_result = requests.get(
            url=ACADEMIC_ANALYTICS_API_BASE_ADDRESS + f"{person_id}/proceedings", headers=headers
        )
