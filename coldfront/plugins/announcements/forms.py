...
class AnnouncementCreateForm(forms.Form):
    ...
    categories = forms.ModelMultipleChoiceField(queryset=AnnouncementCategoryChoice.objects.all(), required=False)
    mailing_lists = forms.ModelMultipleChoiceField(queryset=AnnouncementMailingListChoice.objects.all(), required=False)
