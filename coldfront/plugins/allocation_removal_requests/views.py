...
class AllocationRemovalRequestView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        messages.error(self.request, "You do not have permission to remove this allocation from this project.")
        return False
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if allocation_obj.status.name not in [
            ...
            messages.error(request, f'Cannot remove an allocation with status "{allocation_obj.status}"')
    ...
    def post(self, request, *args, **kwargs):
        ...
        send_allocation_admin_email(
            ...
            addtl_context={"requestor": self.request.user, "project": allocation_obj.project},
        )
        allocation_removal_request.send(
            sender=self.__class__, allocation_removal_request_pk=allocation_removal_request_obj.pk
        )
...
class AllocationRemoveView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["attributes"] = allocation_obj.allocationattribute_set.filter(
...
class AllocationRemovalListView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        messages.error(self.request, "You do not have permission to view allocation removal requests.")

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        if self.request.user.is_superuser:
            allocation_removal_list = AllocationRemovalRequest.objects.filter(status__name="Pending")
        else:
            allocation_removal_list = AllocationRemovalRequest.objects.filter(
                status__name="Pending", allocation__resources__review_groups__in=list(self.request.user.groups.all())
            )
...
class AllocationApproveRemovalRequestView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def test_func(self):
        ...
        messages.error(self.request, "You do not have permission to approve this allocation removal request.")
    ...
    def get(self, request, pk):
        ...
        create_admin_action(request.user, {"status": allocation_status_obj}, allocation_obj)
        ...
        messages.success(request, f'Allocation has been removed from project "{allocation_obj.project.title}"')
...
class AllocationDenyRemovalRequestView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def test_func(self):
        ...
        messages.error(self.request, "You do not have permission to deny this allocation removal request.")
    ...
    def get(self, request, pk):
        ...
        allocation_status_obj = AllocationStatusChoice.objects.get(name=allocation_removal_obj.allocation_prior_status)
        ...
        messages.success(request, f'Allocation has not been removed from project "{allocation_obj.project.title}"')
