        ...
        if allocation_search_form.is_valid():
            ...
            if data.get('show_all_allocations') and self.request.user.is_superuser:
                ...
                allocations = Allocation.objects.prefetch_related('project', 'project__pi', 'status',).filter(
                    ...
                    Q(allocationuser__status__name__in= ['Active', 'Pending - Remove', 'Invited', 'Pending', 'Disabled', 'Retired', "PendingEULA"]))
            ...
            if data.get("username"):
                allocations = allocations.filter(
                    ...
                    Q(allocationuser__status__name__in= ["PendingEULA", 'Active', 'Invited', 'Pending', 'Disabled', 'Retired'])
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            if removed_user_objs:
                ...
                messages.success(
                    request, f"Removed {user_plural} {', '.join(removed_users)} from allocation."
                )
...
class AllocationChangeDetailView(LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not not self.request.user.has_perm("allocation.can_view_all_allocations") and not self.request.user.is_superuser:
            allocation_change_form.fields["end_date_extension"].disabled = True
